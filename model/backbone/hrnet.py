...
from model.ops import build_conv_layer, build_activation_layer, build_norm_layer, build_padding_layer,\
    BasicBlock,Bottleneck,get_expansion
        ...
        for i in range(num_out_branches):
            ...
            for j in range(num_branches):
                if j > i:
                    ...
                    conv_downsamples = []
                    ########需要添加i-j个下采样层，但是上采样为一次性过程
                    for k in range(i - j):
                        #######只相差一层时，需要调整通道为输出通道，且不需要激活
                        if k == i - j - 1:
...
class HRNet(nn.Module):
    ...
    def _make_transition_layer(self, num_channels_pre_layer,
        ...
        for i in range(num_branches_cur):
            # print("branch",i)
            ###########表示与前一阶段共存的branch,此刻不用下采样，只需要维度变换
